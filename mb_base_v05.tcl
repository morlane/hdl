...
set MB_BRAM_RANGE      ${MB_BRAM_SIZE_KB}K
...
# Force MicroBlaze local memory ranges.
# These segment names are stable before grouping on Vivado 2017.x/2025.x,
# but keep them non-fatal for cross-version TCL portability.
set dlmb_mem_seg [get_bd_addr_segs -quiet microblaze_0/Data/SEG_dlmb_bram_if_cntlr_Mem]
if {[llength $dlmb_mem_seg] != 0} {
    set_property range $MB_BRAM_RANGE $dlmb_mem_seg
}

set ilmb_mem_seg [get_bd_addr_segs -quiet microblaze_0/Instruction/SEG_ilmb_bram_if_cntlr_Mem]
if {[llength $ilmb_mem_seg] != 0} {
    set_property range $MB_BRAM_RANGE $ilmb_mem_seg
}
...
# ============================================================
group_bd_cells microblaze_0_local_memory \
    [get_bd_cells ilmb_bram_if_cntlr] \
    [get_bd_cells dlmb_bram_if_cntlr] \
    [get_bd_cells ilmb_v10_0] \
    [get_bd_cells dlmb_v10_0] \
    [get_bd_cells lmb_bram]

# Group pin names can vary after grouping. Keep cosmetic rename operations non-fatal.
catch {set_property name ILMB [get_bd_intf_pins microblaze_0_local_memory/LMB_M]}
catch {set_property name DLMB [get_bd_intf_pins microblaze_0_local_memory/LMB_M1]}
catch {set_property name LMB_Clk [get_bd_pins microblaze_0_local_memory/LMB_Clk]}

group_bd_cells microblaze_0_modules \
...
puts "----------------------------------------------"
puts "        mb_base v05 block design created.     "
puts "                                              "
